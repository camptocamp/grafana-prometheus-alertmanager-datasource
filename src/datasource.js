...
export class GenericDatasource {
  ...
  metricFindQuery(query) {
    ...
    if (query) {
      ...
      if (query) {
        query = encodeURIComponent(
          this.templateSrv.replace(query, {}, this.interpolateQueryExpr) || ""
        );
      }
    }
    let unique = new Set();
    ...
    return this.backendSrv
      .datasourceRequest({
        url:
          this.url +
          "/api/v1/alerts?silenced=false&inhibited=false&filter=" +
          query,
        method: "GET",
      })
      .then((response) => {
        response.data.data.forEach((value) => {
          if (matchedFunction.type === "key") {
            ...
            value =
              value[matchedFunction.matches[1] + "s"][
                matchedFunction.matches[3]
              ];
          }
  ...
  query(options) {
    ...
    if (query.targets[0].type === "table") {
      var labelSelector = this.parseLabelSelector(
        query.targets[0].labelSelector
      );
      return this.backendSrv
        .datasourceRequest({
          url:
        ...
        .then((response) => {
          ...
          if (
            ...
            let columnsDict = this.getColumnsDict(
              response.data.data,
              labelSelector
            );
              ...
              for (let label of Object.keys(item["labels"])) {
                if (label in columnsDict) {
                  if (label === this.severityLabelName) {
                    row[columnsDict[label]] = this.severityLevels[
                      item["labels"][label]
                    ];
                  } else {
  ...
  getColumnsDict(data, labelSelector) {
    ...
    for (let i = 0; i < data.length; i++) {
      for (
        let labelIndex = 0;
        labelIndex < labelSelector.length;
        labelIndex++
      ) {
